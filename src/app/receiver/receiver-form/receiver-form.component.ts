  ...
  async onSubmit() {
    if (this.patientForm.valid) {
      ...
      const { data: sessionData, error: sessionError } =
        await this.receiverFormService.auth.getSession();
      if (
        sessionError ||
        !sessionData ||
        !sessionData.session ||
      ...
      console.log('recieverdata', userFormData);
      if (this.isSubmitting) return; // Stop duplicate submissions
      this.isSubmitting = true;

      console.log(' Submitting form...');

      this.receiverFormService.falseReceiver(userId).subscribe({
        next: (response: any) => {
        ...
        error: (error: any) => {
          console.log('error while setting receiver as false');
        },
      });
    }
  }
